		...
		var methods = {
			...
			go_to: function(index, easing, webkit){
				...
				if(webkit){
					...
					if(settings.direction == 'vertical'){
						$this.data('ppos', (index - 1) * properties.height * (-1));
					}else{
...
//					2014年7月28日 10:21:44  update
					...
					if(settings.direction == 'vertical'){
						...
						//lh update 2014年7月28日 15:32:09
						var poor=event_delta.y>0?200:-200;
						var pos = tinit_pos + tdelta.y + speed.y * settings.inertia+poor;
						
						//console.log(tinit_pos + "   "+tdelta.y +"   "+ speed.y +"  "+ settings.inertia)
						...
						$this.css({
							'-webkit-transition-duration': Math.abs(speed.x * settings.inertia * 3) + 'ms',
							'-webkit-transition-timing-function': 'ease-out'
						});
